...
main_menu.row("🚗 Оценка стоимости ремонта", "💬 FAQ")
...
main_menu.row( "☁️ Погода", "🚦 ПДД и штрафы")
...
@bot.message_handler(func=lambda msg: msg.text == "🚦 ПДД и штрафы")
def show_pdd_fines(msg):
    db_handler.add_active_user(msg.chat.id)
    fines_info = """
    📢 *Актуальные штрафы за нарушения ПДД в Беларуси \\(2025 г\\.\\)\\:*

    *Превышение скорости\\:*
    \\- 10\\-20 км/ч\\: до 42 BYN \\(1 БВ\\)
    \\- 20\\-30 км/ч\\: 42\\-126 BYN \\(1\\-3 БВ\\)
    \\- 30\\-40 км/ч\\: 126\\-420 BYN \\(3\\-10 БВ\\)
    \\- \\>40 км/ч\\: 336\\-504 BYN \\(8\\-12 БВ\\), возможна конфискация ТС при \\>60 км/ч

    *Отсутствие техосмотра\\:*
    \\- Первый раз\\: предупреждение или до 126 BYN \\(до 3 БВ\\)
    \\- Повторно в год\\: 84\\-210 BYN \\(2\\-5 БВ\\), возможно лишение прав на 6 месяцев

    *Отсутствие страховки\\:*
    \\- 168\\-252 BYN \\(4\\-6 БВ\\), эвакуация ТС возможна

    *Непристегнутый ремень\\:*
    \\- Водитель\\: 42 BYN \\(1 БВ\\)
    \\- Пассажиры\\: 21\\-42 BYN \\(0\\.5\\-1 БВ\\)

    *Управление в состоянии опьянения\\:*
    \\- Алкоголь \\(\\>0\\.3 ‰\\)\\: 504\\-1008 BYN \\(12\\-24 БВ\\) \\+ лишение прав на 3\\-5 лет
    \\- Наркотики\\: 1008\\-2520 BYN \\(24\\-60 БВ\\) \\+ лишение прав на 5\\-10 лет

    *Использование телефона за рулем\\:*
    \\- 84\\-126 BYN \\(2\\-3 БВ\\)

    *Нарушение правил обгона\\:*
    \\- 126\\-252 BYN \\(3\\-6 БВ\\), возможна эвакуация

    *Проезд на красный свет\\:*
    \\- 126\\-336 BYN \\(3\\-8 БВ\\)

    ℹ️ *Дополнительная информация\\:*
    Актуальная базовая величина \\(БВ\\) в 2025 году — 42 BYN\\. Полную информацию о ПДД и штрафах найдете в Кодексе об административных правонарушениях РБ \\([pravo\\.by](https\\://pravo\\.by/document/\\?guid\\=3871\\&p0\\=C22100049)\\) или у сотрудников ГАИ\\.
    """
    bot.send_message(
        msg.chat.id,
        fines_info,
...
@bot.message_handler(func=lambda msg: msg.text == "🚗 Оценка стоимости ремонта")
def estimate_repair_cost(msg):
    db_handler.add_active_user(msg.chat.id)
    sent_msg = bot.send_message(
        msg.chat.id,
        "🚗 Выберите тип ремонта или нажмите 'Другое' для ввода своей поломки:",
        reply_markup=repair_menu
    )
    bot.register_next_step_handler(sent_msg, process_repair_estimate)


def process_repair_estimate(msg):
    if msg.text == "❌ Отмена":
        return_to_main(msg)
        return
    user_input = msg.text.strip().lower()
    # Основные данные для оценок (в BYN, включая работу и запчасти)
    repair_costs = {
        "замена тормозных колодок": "50-100 BYN (зависит от марки автомобиля)",
        "замена тормозных дисков": "100-200 BYN (включая работу)",
        "замена масла": "30-60 BYN (с учетом фильтра и работы)",
        "замена масляного фильтра": "20-40 BYN",
        "ремонт стартера": "80-150 BYN",
        "замена стартера": "120-200 BYN",
        "замена свечей зажигания": "40-80 BYN",
        "ремонт подвески": "100-200 BYN (зависит от деталей)",
        "замена амортизаторов": "80-150 BYN за пару",
        "ремонт рулевого управления": "150-300 BYN",
        "замена рулевых тяг": "70-120 BYN",
        "замена аккумулятора": "70-120 BYN",
        "диагностика": "20-40 BYN",
        "замена воздушного фильтра": "20-50 BYN",
        "ремонт генератора": "100-250 BYN",
        "замена ремня грм": "80-180 BYN (включая работу)",
        "ремонт выхлопной системы": "100-300 BYN (зависит от повреждений)",
        "замена охлаждающей жидкости": "30-70 BYN",
        "ремонт радиатора": "100-200 BYN",
        "замена стеклоочистителей": "20-40 BYN",
        "ремонт коробки передач": "200-500 BYN (зависит от типа кпп)",
        "замена сцепления": "150-300 BYN",
        "ремонт тормозной системы": "100-250 BYN",
        "замена глушителя": "80-150 BYN",
        "ремонт электрики": "50-150 BYN (зависит от сложности)"
    }
    # Словарь синонимов для разных вариантов написания
    repair_synonyms = {
        "замена тормозных колодок": ["поменять колодки", "замена колодок", "ремонт колодок", "смена колодок"],
        "замена тормозных дисков": ["поменять диски", "замена дисков", "ремонт дисков", "смена дисков"],
        "замена масла": ["поменять масло", "смена масла", "масляная смена"],
        "замена масляного фильтра": ["поменять масляный фильтр", "замена фильтра масла", "смена масляного фильтра"],
        "ремонт стартера": ["починить стартер", "ремонт стартёра", "починка стартера"],
        "замена стартера": ["поменять стартер", "смена стартера", "замена стартёра"],
        "замена свечей зажигания": ["поменять свечи", "замена свечей", "смена свечей", "ремонт свечей"],
        "ремонт подвески": ["починить подвеску", "ремонт подушки", "починка подвески"],
        "замена амортизаторов": ["поменять амортизаторы", "смена амортизаторов", "замена амортиков"],
        "ремонт рулевого управления": ["починить руль", "ремонт рулевого", "починка руля"],
        "замена рулевых тяг": ["поменять тяги", "смена рулевых тяг", "замена тяг рулевых"],
        "замена аккумулятора": ["поменять аккумулятор", "смена аккумулятора", "замена акб"],
        "диагностика": ["проверка авто", "диагностика машины", "проверка состояния"],
        "замена воздушного фильтра": ["поменять воздушный фильтр", "смена воздушного фильтра",
                                      "замена фильтра воздуха"],
        "ремонт генератора": ["починить генератор", "ремонт генаратора", "починка генератора"],
        "замена ремня грм": ["поменять ремень грм", "смена ремня грм", "замена ремня газораспределения"],
        "ремонт выхлопной системы": ["починить выхлоп", "ремонт выхлопа", "починка выхлопной"],
        "замена охлаждающей жидкости": ["поменять охлаждающую жидкость", "смена антифриза", "замена тосола"],
        "ремонт радиатора": ["починить радиатор", "ремонт радиатора системы", "починка радиатора"],
        "замена стеклоочистителей": ["поменять дворники", "смена стеклоочистителей", "замена дворников"],
        "ремонт коробки передач": ["починить коробку", "ремонт кпп", "починка коробки передач"],
        "замена сцепления": ["поменять сцепление", "смена сцепления", "ремонт сцепления"],
        "ремонт тормозной системы": ["починить тормоза", "ремонт тормозов", "починка тормозной"],
        "замена глушителя": ["поменять глушитель", "смена глушителя", "замена выхлопа"],
        "ремонт электрики": ["починить электрику", "ремонт проводки", "починка электрики"]
    }

    if user_input == "другое":
        sent_msg = bot.send_message(
            msg.chat.id,
            "🚗 Опишите поломку (например, 'ремонт коробки передач', 'замена ремня ГРМ') или выберите '❌ Отмена':",
            reply_markup=cancel_menu
        )
        bot.register_next_step_handler(sent_msg, process_custom_repair)
    else:
        # Поиск точного совпадения
        if user_input in repair_costs:
            bot.send_message(
                msg.chat.id,
                f"💰 Примерная стоимость ремонта '{msg.text}': {repair_costs[user_input]}.\nЦены могут варьироваться в зависимости от марки автомобиля, региона и использования оригинальных запчастей. Для точной оценки обратитесь на СТО.",
                reply_markup=main_menu
            )
        else:
            # Поиск среди синонимов
            matched_repair = None
            for key, synonyms in repair_synonyms.items():
            ...
            if matched_repair:
                bot.send_message(
                    msg.chat.id,
                    f"💰 Примерная стоимость ремонта '{msg.text}': {repair_costs[matched_repair]}.\nЦены могут варьироваться в зависимости от марки автомобиля, региона и использования оригинальных запчастей. Для точной оценки обратитесь на СТО.",
                    reply_markup=main_menu
                )
            else:
                sent_msg = bot.send_message(
                    msg.chat.id,
                    f"❗ '{msg.text}' не распознано. Опишите поломку или выберите '❌ Отмена'.",
                    reply_markup=cancel_menu
                )
                bot.register_next_step_handler(sent_msg, process_custom_repair)


def process_custom_repair(msg):
    if msg.text == "❌ Отмена":
        return_to_main(msg)
        return
    user_input = msg.text.strip().lower()
    ...
    if user_input in repair_costs:
        ...
        if matched_repair:
            ...
            sent_msg = bot.send_message(
                msg.chat.id,
                f"❗ '{msg.text}' не распознано. Попробуйте снова или выберите '❌ Отмена'.",
                reply_markup=cancel_menu
            )
            bot.register_next_step_handler(sent_msg, process_custom_repair)
